    ...
    <div className="p-4 md:p-8 w-full max-w-7xl mx-auto transition-colors duration-200">
      ...
      <header className="mb-8">
        <div className="flex items-center gap-3 mb-2">
           ...
           <h2 className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-slate-100">Analytics & Reports</h2>
        </div>
        <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm md:text-base font-medium">Track your mental wellness progress over time.</p>
      </header>
      ...
      {isLoading ? (
        ...
        <div className="bg-white dark:bg-slate-900 p-12 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col items-center justify-center text-center h-[50vh] transition-colors">
          <div className="w-20 h-20 bg-blue-50 dark:bg-blue-950/40 rounded-full flex items-center justify-center mb-4 border border-blue-100 dark:border-blue-900/50">
            <svg className="w-10 h-10 text-blue-300 dark:text-blue-500" fill="currentColor" viewBox="0 0 24 24"><path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zM9 17H7v-7h2v7zm4 0h-2V7h2v10zm4 0h-2v-4h2v4z"/></svg>
          </div>
          <h4 className="text-lg font-bold text-slate-700 dark:text-slate-200 mb-2">Not enough data</h4>
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 transition-colors">
              <div className="flex items-center justify-between mb-8">
                ...
                <div className="p-2 bg-indigo-50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400 rounded-lg border border-indigo-100 dark:border-indigo-900/50">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden transition-colors">
             <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
               <h3 className="font-bold text-slate-800 dark:text-slate-100 text-lg">Check-in History Log</h3>
             </div>
               ...
               <table className="w-full text-left border-collapse">
                 ...
                 <tbody className="text-sm divide-y divide-slate-100 dark:divide-slate-800">
                   {checkins.map((checkin, idx) => (
                     <tr key={checkin.id || idx} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition">
                       ...
                       <td className="p-4">
                         <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                           checkin.mood === 'Great' ? 'bg-green-100 dark:bg-green-950/40 text-green-700 dark:text-green-300 border border-green-200 dark:border-green-800/50' :
                           checkin.mood === 'Good' ? 'bg-blue-100 dark:bg-blue-950/40 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800/50' :
                           checkin.mood === 'Okay' ? 'bg-yellow-100 dark:bg-yellow-950/40 text-yellow-700 dark:text-yellow-300 border border-yellow-200 dark:border-yellow-800/50' :
                           'bg-red-100 dark:bg-red-950/40 text-red-700 dark:text-red-300 border border-red-200 dark:border-red-800/50'
                         }`}>
                           {checkin.mood || "N/A"}
                         </span>
                       </td>
                       <td className="p-4 text-slate-600 dark:text-slate-400 font-semibold">{checkin.stressLevel || "0"}</td>
                       <td className="p-4 text-slate-600 dark:text-slate-400 font-semibold">{checkin.sleepHours || "0"} hrs</td>
                     </tr>
                   ))}
                 </tbody>
               </table>
...
function MetricCard({ title, value, subtitle, icon, color }: { title: string, value: string, subtitle: string, icon: string, color: string }) {
  const colorMap: Record<string, string> = {
    ...
    red: "bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 border-red-100 dark:border-red-800/50",
    green: "bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400 border-green-100 dark:border-green-800/50",
  };

  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 shadow-sm border border-slate-100 dark:border-slate-800 flex items-start gap-4 transition-colors">
      ...
      <div>
        ...
        <h3 className="text-2xl font-black text-slate-800 dark:text-slate-100 leading-none mb-1">{value}</h3>
