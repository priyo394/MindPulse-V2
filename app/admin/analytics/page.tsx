    ...
    <div className="flex-1 flex flex-col overflow-y-auto bg-[#f8fafc] dark:bg-slate-950 p-4 md:p-6 space-y-6 transition-colors">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl shadow-sm transition-colors">
            <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-1">User Growth Trend</h3>
            <p className="text-xs text-slate-400 dark:text-slate-500 mb-6">Visual representation of daily registrations.</p>
              ...
              {growthData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={growthData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                    ...
                    <Tooltip 
                      contentStyle={{ backgroundColor: 'var(--tw-prose-bg, #ffffff)', borderColor: '#cbd5e1', borderRadius: '10px', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }}
                      wrapperStyle={{ color: '#000' }}
                    />
                  </LineChart>
                </ResponsiveContainer>
              ) : (
                <div className="w-full h-full flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 bg-slate-50 dark:bg-slate-800/40 rounded-xl border border-dashed border-slate-200 dark:border-slate-700">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl shadow-sm transition-colors">
            <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-1">Check-in Distributions</h3>
            <p className="text-xs text-slate-400 dark:text-slate-500 mb-6">Mood trends breakdown over time.</p>
